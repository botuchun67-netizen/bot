...
# ===================== SOZLAMALAR =====================
TOKEN = os.getenv("BOT_TOKEN", "8708873894:AAH_1taP4ysWf2ZtgpQ46LPFoQtdr6HbHrA")
ADMIN_ID = 8355611778
...
def init_db():
    cols = [r[1] for r in allr("PRAGMA table_info(votes)")]
    if cols and "voter_key" not in cols:  # eski versiya jadvali
        ex("ALTER TABLE votes RENAME TO votes_old")
    _con.executescript("""
...
async def show(q, text, kb=None):
    """Xabarni tahrirlaydi; rasmli xabar bo'lsa yoki tahrirlab bo'lmasa yangisini yuboradi."""
    try:
...
async def show_cat(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    cands = allr("SELECT id, name, photo FROM candidates WHERE cat=? ORDER BY id", (cat,))
    if not cands:
        await show(q, f"{title}\n\nBu yo'nalishda hozircha nomzod yo'q.", Kb([BACK_MENU]))
        return
    chat = q.message.chat_id
    # Rasmlar albom qilib (10 tadan) yuboriladi: 13 ta nomzod = 2 ta so'rov
    for i in range(0, len(cands), 10):
        chunk = cands[i:i + 10]
        try:
            if len(chunk) == 1:
                _, name, photo = chunk[0]
                await ctx.bot.send_photo(chat, photo, caption=f"{i + 1}. {name}")
            else:
                media = [InputMediaPhoto(p, caption=f"{i + j + 1}. {n}")
                         for j, (_, n, p) in enumerate(chunk)]
                await ctx.bot.send_media_group(chat, media)
        except Exception:
            log.exception("Rasmlarni yuborib bo'lmadi")
    rows = [[Btn(f"{i}. {name}", callback_data=f"v:{cid}")] for i, (cid, name, _) in enumerate(cands, 1)]
    rows.append(BACK_MENU)
    await ctx.bot.send_message(chat, f"📌 {title}\n\nOvoz bermoqchi bo'lgan nomzodni tanlang:",
...
async def vote(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    cand = one("SELECT name FROM candidates WHERE id=?", (cid,))
    if not cand:
...
async def admin_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if act == "panel":
        ...
        n = int(p[2])
        rows = allr("""SELECT c.id, c.name, c.photo, COUNT(v.id) FROM candidates c
...
async def admin_photo(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    ex("INSERT INTO candidates(cat, name, photo) VALUES(?,?,?)",
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(vote, pattern=r"^(?:v|vote):\d+$"))
